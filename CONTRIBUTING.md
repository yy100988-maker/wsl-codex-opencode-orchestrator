...
- 版本格式：`X.Y.Z`（SemVer 简化版），当前版本 `0.1.1`。
- 只有代码或运行契约改动才递增第三位：`0.1.0` → `0.1.1` → `0.1.2`，依此类推。代码或运行契约指 `scripts/`、`tests/`、以及 `SKILL.md` 的行为性条款。
- 纯文档更新（README、使用说明、配置参考、复盘等）不升级版本号，直接提交并推送即可。
