...
finished="$(find "$events" -type f -name '*.jsonl' -print0 | xargs -0 -r cat | jq -s '[.[] | select(.event_type == "task-finished") | {task_id,status:(.data.status // "blocked"),reason:(.data.reason // null),at:.timestamp}]')"
